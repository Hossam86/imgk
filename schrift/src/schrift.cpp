...
midpoint(struct point a, struct point b) {
    point result{};
    result.x = 0.5 * a.x + 0.5 * b.x;
    result.y = 0.5 * a.y + 0.5 * b.y;
    return result;
};
...
decode_contour(uint8_t *flags, unsigned int basePoint, unsigned int count, struct outline *outl) {
        ...
        if (flags[i] & POINT_IS_ON_CURVE) {
            if (gotCtrl) {
                if (outl->numCurves >= outl->capCurves && grow_curves(outl) < 0)
                    return -1;
                outl->curves[outl->numCurves++] = (struct curve) {static_cast<uint_least16_t>(beg),
                                                                  static_cast<uint_least16_t>(basePoint + i),
                                                                  static_cast<uint_least16_t>(ctrl)};
            } else {
                if (outl->numLines >= outl->capLines && grow_lines(outl) < 0)
                    return -1;
                outl->lines[outl->numLines++] = (struct line) {static_cast<uint_least16_t>(beg),
                                                               static_cast<uint_least16_t>(basePoint + i)};
            }
            beg = basePoint + i;
            gotCtrl = 0;
            ...
            if (gotCtrl) {
                ...
                if (outl->numCurves >= outl->capCurves && grow_curves(outl) < 0)
                    return -1;
                outl->curves[outl->numCurves++] = (struct curve) {static_cast<uint_least16_t>(beg),
                                                                  static_cast<uint_least16_t>(center),
                                                                  static_cast<uint_least16_t>(ctrl)};
    ...
    if (gotCtrl) {
        if (outl->numCurves >= outl->capCurves && grow_curves(outl) < 0)
            return -1;
        outl->curves[outl->numCurves++] = (struct curve) {static_cast<uint_least16_t>(beg),
                                                          static_cast<uint_least16_t>(looseEnd),
                                                          static_cast<uint_least16_t>(ctrl)};
    } else {
        if (outl->numLines >= outl->capLines && grow_lines(outl) < 0)
            return -1;
...
#define STACK_SIZE 10
    ...
    for (;;) {
        if (is_flat(outl, curve, 0.5) || top >= STACK_SIZE) {
            ...
            stack[top++] = (struct curve) {curve.beg, static_cast<uint_least16_t>(pivot),
                                           static_cast<uint_least16_t>(ctrl0)};
